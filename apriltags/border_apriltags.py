...
def main():
    ...
    # TODO: Original code. Didn't work due to the paths not existing, and also paths being formatted for Mac/Linux.
    ...
    # TODO: These lines were changed by me.
    project_directory = Path(__file__).parent.parent.parent
    output_path = str(project_directory.joinpath('data', 'output'))
    out_video = output_path + '\\{}_with_apriltags.mp4'.format(os.path.basename(src_video)[:-4])
    out_img = output_path + '\\{}_grid.png'.format(os.path.basename(src_video)[:-4])
    out_img_with_labels = output_path + '\\{}_grid_with_labels.png'.format(os.path.basename(src_video)[:-4])

    # TODO: Test. Remove.
    print(f"\nout_video: {out_video}\nout img: {out_img}\nout img with labels: {out_img_with_labels}")
